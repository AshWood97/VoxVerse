...
function Test-AgentStdioSmoke {
  Write-Host "== voxverse agent stdio"
  ...
  try {
    ...
    if ($exitCode -eq 0 -and $output -match '"id":"characters"' -and $output -match '"characters"' -and $output -match '"id":"stop"' -and $output -match '"shutdown":true') {
      Add-Check "voxverse agent stdio" "pass" (($output.Trim() -split "`r?`n" | Select-Object -Last 8) -join "`n")
    } else {
      Add-Check "voxverse agent stdio" "fail" (($output.Trim() -split "`r?`n" | Select-Object -Last 12) -join "`n")
    }
  } catch {
    Add-Check "voxverse agent stdio" "fail" $_.Exception.Message
  } finally {
    Pop-Location
  }
}
...
function Test-McpStdioSmoke {
  ...
  try {
    ...
    if (
      ...
      Add-Check "voxverse mcp stdio" "pass" (($output.Trim() -split "`r?`n" | Select-Object -Last 8) -join "`n")
    } else {
      Add-Check "voxverse mcp stdio" "fail" (($output.Trim() -split "`r?`n" | Select-Object -Last 12) -join "`n")
    }
  } catch {
    Add-Check "voxverse mcp stdio" "fail" $_.Exception.Message
  } finally {
    Pop-Location
  }
}
...
function Test-AgentWriteSmoke {
  Write-Host "== voxverse agent write guard"
  $sourceDb = Get-VoxVerseDbCandidate
  if ([string]::IsNullOrWhiteSpace($sourceDb)) {
    Add-Check "voxverse agent write guard" "warn" "No local VoxVerse database was found, so the agent write smoke test was skipped."
    return
  }

  $tmpRoot = Join-Path ([System.IO.Path]::GetTempPath()) "voxverse-agent-write-smoke"
  ...
  $auditLog = Join-Path $tmpRoot "voxverse-audit.jsonl"
  ...
  try {
    ...
    try {
      ...
      if (
        ...
        -and $writeOutput -match '"actor":"voxverse-agent"' `
        -and $writeOutput -match '"shutdown":true'
      ) {
        Add-Check "voxverse agent write guard" "pass" (($combinedOutput.Trim() -split "`r?`n" | Select-Object -Last 8) -join "`n")
      } else {
        Add-Check "voxverse agent write guard" "fail" (($combinedOutput.Trim() -split "`r?`n" | Select-Object -Last 12) -join "`n")
      }
    } finally {
      Pop-Location
    }
  } catch {
    Add-Check "voxverse agent write guard" "fail" $_.Exception.Message
  } finally {
    if (Test-Path $tmpDb) {
...
function Test-SendMessageMockSmoke {
  Write-Host "== voxverse send-message mock"
  $sourceDb = Get-VoxVerseDbCandidate
  if ([string]::IsNullOrWhiteSpace($sourceDb)) {
    Add-Check "voxverse send-message mock" "warn" "No local VoxVerse database was found, so the send-message mock smoke test was skipped."
    return
  }

  $tmpRoot = Join-Path ([System.IO.Path]::GetTempPath()) "voxverse-send-message-smoke"
  if (-not (Test-Path $tmpRoot)) {
    New-Item -ItemType Directory -Path $tmpRoot | Out-Null
  }
  ...
  $auditLog = Join-Path $tmpRoot "voxverse-audit.jsonl"
  $mockJob = $null
  $oldBaseUrl = $env:VOXVERSE_CLI_SEND_BASE_URL
  $oldProvider = $env:VOXVERSE_CLI_SEND_PROVIDER
  $oldModel = $env:VOXVERSE_CLI_SEND_MODEL
  $oldApiKey = $env:VOXVERSE_CLI_SEND_API_KEY
  ...
  try {
    ...
    try {
      ...
      if ($LASTEXITCODE -ne 0) {
        Add-Check "voxverse send-message mock" "fail" (($characterOutput.Trim() -split "`r?`n" | Select-Object -Last 8) -join "`n")
        return
      }
      ...
      if ($characters.Count -eq 0) {
        Add-Check "voxverse send-message mock" "warn" "No local characters were found, so the send-message mock smoke test was skipped."
        return
      }
      $characterId = [string]$characters[0].id
      ...
      if ($LASTEXITCODE -ne 0) {
        Add-Check "voxverse send-message mock" "fail" (($sessionOutput.Trim() -split "`r?`n" | Select-Object -Last 8) -join "`n")
        return
      }
      $sessionReport = $sessionOutput | ConvertFrom-Json
      $sessionId = [string]$sessionReport.session.id
      ...
      $env:VOXVERSE_CLI_SEND_BASE_URL = "http://127.0.0.1:$port/v1"
      $env:VOXVERSE_CLI_SEND_PROVIDER = "custom"
      $env:VOXVERSE_CLI_SEND_MODEL = "lv4-mock-model"
      $env:VOXVERSE_CLI_SEND_API_KEY = ""
      ...
      $filteredAuditOutput = & cargo run --quiet --bin voxverse-cli -- --db "$tmpDb" audit list --json --limit 8 --operation practice.message.send --actor voxverse-cli --result success --session "$sessionId" 2>&1 | Out-String
      $filteredAuditExit = $LASTEXITCODE
      $retryAuditOutput = & cargo run --quiet --bin voxverse-cli -- --db "$tmpDb" audit list --json --limit 8 --operation practice.message.retry_last --actor voxverse-cli --result success --session "$sessionId" 2>&1 | Out-String
      $retryAuditExit = $LASTEXITCODE
      $agentRetryAuditOutput = & cargo run --quiet --bin voxverse-cli -- --db "$tmpDb" audit list --json --limit 8 --operation practice.message.retry_last --actor voxverse-agent --result success --session "$sessionId" 2>&1 | Out-String
      ...
      if (
        ...
        -and $filteredAuditOutput -match '"actor"\s*:\s*"voxverse-cli"' `
        -and $retryAuditOutput -match '"operation"\s*:\s*"practice.message.retry_last"' `
        -and $agentRetryAuditOutput -match '"actor"\s*:\s*"voxverse-agent"' `
        -and $filteredAuditOutput -match $targetSessionPattern
      ) {
        Add-Check "voxverse send-message mock" "pass" (($combinedOutput.Trim() -split "`r?`n" | Select-Object -Last 10) -join "`n")
      } else {
        Add-Check "voxverse send-message mock" "fail" (($combinedOutput.Trim() -split "`r?`n" | Select-Object -Last 14) -join "`n")
      }
    } finally {
      Pop-Location
    }
  } catch {
    Add-Check "voxverse send-message mock" "fail" $_.Exception.Message
  } finally {
    $env:VOXVERSE_CLI_SEND_BASE_URL = $oldBaseUrl
    $env:VOXVERSE_CLI_SEND_PROVIDER = $oldProvider
    $env:VOXVERSE_CLI_SEND_MODEL = $oldModel
    $env:VOXVERSE_CLI_SEND_API_KEY = $oldApiKey
...
function Get-VoxVerseDbCandidate {
  $candidates = @()
  if (-not [string]::IsNullOrWhiteSpace($env:VOXVERSE_DB)) {
    $candidates += $env:VOXVERSE_DB
  }
...
function Test-ProfileSwitchSmoke {
  Write-Host "== voxverse profile switch"
  $sourceDb = Get-VoxVerseDbCandidate
  if ([string]::IsNullOrWhiteSpace($sourceDb)) {
    Add-Check "voxverse profile switch" "warn" "No local VoxVerse database was found, so the write-operation smoke test was skipped."
    return
  }

  $tmpRoot = Join-Path ([System.IO.Path]::GetTempPath()) "voxverse-cli-smoke"
  if (-not (Test-Path $tmpRoot)) {
    New-Item -ItemType Directory -Path $tmpRoot | Out-Null
  }
  ...
  try {
    ...
    try {
      ...
      if (
        ...
        Add-Check "voxverse profile switch" "pass" (($combinedOutput.Trim() -split "`r?`n" | Select-Object -Last 8) -join "`n")
      } else {
        Add-Check "voxverse profile switch" "fail" (($combinedOutput.Trim() -split "`r?`n" | Select-Object -Last 12) -join "`n")
      }
    } finally {
      Pop-Location
    }
  } catch {
    Add-Check "voxverse profile switch" "fail" $_.Exception.Message
  } finally {
    if (Test-Path $tmpDb) {
      Remove-Item -LiteralPath $tmpDb -Force
    }
...
Invoke-Check "voxverse cli diagnostics" "cargo run --quiet --bin voxverse-cli -- diagnostics run --json" (Join-Path $root "src-tauri")
Invoke-Check "voxverse cli characters" "cargo run --quiet --bin voxverse-cli -- character list --json" (Join-Path $root "src-tauri")
Invoke-Check "voxverse cli sessions" "cargo run --quiet --bin voxverse-cli -- session list --json" (Join-Path $root "src-tauri")
Invoke-Check "voxverse agent tools" "cargo run --quiet --bin voxverse-cli -- agent tools list --json" (Join-Path $root "src-tauri")
